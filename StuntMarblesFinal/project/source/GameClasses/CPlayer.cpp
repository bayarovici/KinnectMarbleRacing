...
void CPlayer::init(IrrlichtDevice *pDevice, CGame *pGame) {
	...
	m_kinectJoystick=new CKinectJoystickGUI(pDevice->getGUIEnvironment(),rect<s32>(50,20,150,20)); 
	
}

CPlayer::~CPlayer() {
	delete m_pAnimator;
	delete m_kinectJoystick; // HACK
	m_aCpState.clear();
	m_aLapNo.clear();
}
...
void CPlayer::step(bool bDoCamStep) {
	//std::cout <<"NO PLAYERS:" <<m_iNum;
	//	std:: cout <<"KINECT STATE " << kinectWrapper->isLeanFwd;
	// std::cout << kinectWrapper->gesturePt->isLeanFwd;
	f32 f;
	...
	if (m_bActive) {
		f32 f2=m_pGame->getControl(m_bNetClient?1:m_iNum,C_BR);
				...
				//FIX		
		if(m_iNum ==1){ // if this is the first player
			m_kinectJoystick->transmissionData= kinectWrapper->transmissionData;
			m_kinectJoystick->update(kinectWrapper->zMovement1,kinectWrapper->xMovement1);
			f32 acc=0;		
			if(kinectWrapper->isPlayer1LeanFwd){ // MAX VALUE IS 300
				//std::cout<< "FOWARD IS " <<kinectWrapper->zMovement1/2 <<"\n";
				acc=kinectWrapper->zMovement1*0.5;
			}
			//f32 acc=kinectWrapper->isPlayer1LeanFwd* 90;
			//f32 acc=250*(m_pGame->getControl(m_bNetClient?1:m_iNum,C_FW));

			//f32 str=250*(m_pGame->getControl(m_bNetClient?1:m_iNum,C_LF));

			f32 str=0;
			if(kinectWrapper->isPlayer1LeanLeft)
				str= kinectWrapper->xMovement1*-1.5;
				//str = 260;
			if(kinectWrapper->isPlayer1LeanRight)
				str=kinectWrapper->xMovement1*-1.5;
			...
			f=0.005f;
			if(kinectWrapper->zMovement1 <-60)
				f2=1;
		}
		if(m_iNum ==2){ // if this is the second player
			//m_kinectJoystick->plyerNo=m_iNum;
			m_kinectJoystick->update(kinectWrapper->zMovement2,kinectWrapper->xMovement2);
			//m_kinectJoystick->update(kinectWrapper->frontLean,0);
			//  std::cout << "\n" << kinectWrapper->frontLean;
			f32 acc=0;
			if(kinectWrapper->isPlayer1LeanFwd){ // value > 100
				//std::cout<< "FOWARD IS " <<kinectWrapper->zMovement1/2 <<"\n";
				acc=kinectWrapper->zMovement2/2;
			}
			//f32 acc=250*(m_pGame->getControl(m_bNetClient?1:m_iNum,C_FW));
			...
			f32 str=0;
			if(kinectWrapper->isPlayer2LeanLeft)
				str= kinectWrapper->xMovement1*-1.5;
			if(kinectWrapper->isPlayer2LeanRight)
				str=kinectWrapper->xMovement2*-1.5;
			if (acc>=25.0f || acc<=-25.0f || str>=25.0f || str<=-25.0f) {
				f32 a=m_pAnimator->getAngleH();
				vector3df push=vector3df(0.0f,a,0.0f).rotationToDirection(-vector3df(acc,0.0f,str));
				m_pController->addTorque(m_pMarble,push);
			...
			if(kinectWrapper->zMovement2 <-60)
				f2=1; //break
...
void CPlayer::updateGUI(u32 iTime) {
	if (m_bActive) {
		...
		if (m_iRespawnTime)
			...
			swprintf(s,0xFF,L"Time: %.2f  Speed: %.2f",((f32)iTime)/1000,m_pMarble->getLinearVelocity().getLength());
